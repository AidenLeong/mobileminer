...
$signed= hash_hmac('sha256', $signing, 'S-WgWnPqBPnTumoF4Be9MHSA');
if ($signed === $data['x_signature']) {

    if ($paidstatus == "Success"){ //payment success
        
        $sqlcart = "SELECT ID,CQUANTITY FROM CART WHERE EMAIL = '$userid'";
            ...
            $productresult = $conn->query($selectproduct);
             if ($productresult->num_rows > 0){
                  while ($rowp = $productresult->fetch_assoc()){
                    ...
                    $newsold = $prevsold + $cq;
                    $sqlupdatequantity = "UPDATE PRODUCT SET QUANTITY = '$newquantity', SOLD = '$newsold' WHERE ID = '$id'";
       ...
       $conn->query($sqlinsert);
        ...
        echo '<br><br><body><div><h2><br><br><center>Receipt</center></h1><table border=1 width=80% align=center><tr><td>Order id</td><td>'.$orderid.'</td></tr><tr><td>Receipt ID</td><td>'.$receiptid.'</td></tr><tr><td>Email to </td><td>'.$userid. ' </td></tr><td>Amount </td><td>RM '.$amount.'</td></tr><tr><td>Payment Status </td><td>'.$paidstatus.'</td></tr><tr><td>Date </td><td>'.date("d/m/Y").'</td></tr><tr><td>Time </td><td>'.date("h:i a").'</td></tr></table><br><p><center>Press back button to return to MobileMiner</center></p></div></body>';
